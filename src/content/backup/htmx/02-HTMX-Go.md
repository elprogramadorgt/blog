...
_ load: Se ejecuta una vez cuando el elemento carga en la pagina web.
_ revealed: Se ejecuta una vez cuando el elemento aparece en el viewport (lo que usuario ve en el explorador) \* intersect: Se ejecuta cuando un un elemento se intersecta con el viewport.
Tiene 2 opciones adicionales: 1. root:<selector> - un selector css del elemento raiz para la interseccion. 2. threshold:<float> - un numero flotante entre 0.0 y 1.1 que indica la que cantidad de interseccion debe ocurrir para que se dispare el evento.
...
```html
<button
  hx-get="http://localhost:8080/alert"
  hx-trigger="revealed"
