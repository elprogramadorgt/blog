<!-- <svelte:head>
  <script bind:this={script} src={"https://cdn.jsdelivr.net/npm/canvas-confetti@1.9.2/dist/confetti.browser.min.js"} />
</svelte:head> -->
        ...
        {#each quiz.questions[currentQuestionIndex].options as option, index}
          <li
            class="option {selectedOption === option
              ? option === quiz.questions[currentQuestionIndex].answer
                ? 'correct'
                : 'incorrect'
              : ''}"
...
<style>
  ...
  .incorrect {
    ...
    background-color: var(
      --quiz-incorrect-bg
    ); /* Reddish background to indicate wrong */
    border-color: var(--quiz-incorrect-border);
  }

  .option.incorrect:hover,
  .option.incorrect:focus {
    background-color: var(
      --quiz-incorrect-bg
    ); /* Lighter shade of red for hover on incorrect */
    border-color: var(
      --quiz-incorrect-border
    ); /* Darker shade of red for border on hover */
  ...
  .results {
    ...
    background-color: var(
      --card-bg-color
    ); /* Ensure it adapts to light/dark themes */
    border-radius: 10px;
    box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);
    margin-top: 2rem;
  }
  ...
  p.result-message {
    ...
    background-color: var(
      --tag-bg-color
    ); /* Using a subtle background for emphasis */
    border-radius: 8px;
  }
